...
{
    public class InternshipapiController : ApiController
    {
        private readonly IInternshipManagementService _managerServices;
        public InternshipapiController(IInternshipManagementService userManagementService)
        {
        ...
        public HttpResponseMessage Get()
        {
            ...
            if (users != null)
            {
                var userEntities = users as List<Internship> ?? users.ToList();
                if (userEntities.Any())
        ...
        // POST api/user
        public string Post([FromBody] Internship userEntity)
        {
            return _managerServices.CreateProject(userEntity);
        }

        // PUT api/user/5
        public bool Put(string id, [FromBody]Internship userEntity)
        {

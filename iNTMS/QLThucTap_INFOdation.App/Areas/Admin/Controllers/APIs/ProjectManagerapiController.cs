...
{
    public class ProjectManagerapiController : ApiController
    {
        private readonly IProjectManagerManagementService _projectmanagerServices;
        public ProjectManagerapiController(IProjectManagerManagementService userManagementService)
        {
        ...
        public HttpResponseMessage Get()
        {
            ...
            if (users != null)
            {
                var userEntities = users as List<ProjectManager> ?? users.ToList();
                if (userEntities.Any())
        ...
        // POST api/user
        public string Post([FromBody] ProjectManager userEntity)
        {
            return _projectmanagerServices.CreateProject(userEntity);
        }

        // PUT api/user/5
        public bool Put(string id, [FromBody]ProjectManager userEntity)
        {
